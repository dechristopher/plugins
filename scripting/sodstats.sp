...
public Plugin:myinfo = 
{
	name = "[KIWI] Stats",
	author = "drop",
	description = "A simple distributed stats and ranking system.",
...
public OnPluginStart()
{
	...
	{
		LogError("[KIWI Stats] Unable to connect to database (%s)", error);
		return;
	}
	...
	{
		LogError("[KIWI Stats] Invalid DB-Type");
		return;
	}
	...
	{
		LogError("[KIWI Stats] Could not create players table.");
		return;
	}
	
	if(!SQL_FastQuery(stats_db, g_sql_addheadshots))
	{
		//LogError("[KIWI-Rank] Could not add headshots column.");
		//return;
	}
	...
	{
		LogError("[KIWI Stats] Unable to hook events.");
		return;
	}
	...
	g_hversion = CreateConVar("sm_stats_version", SODSTATS_VERSION,"KIWI Stats version.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	{
		LogError("[KIWI Stats] Could not create stats_enabled cvar.");
		return;
	}
...
{ 
    if(g_NumClients < 4) {
    	...
    	PrintToChatAll("\x01[\x09KIWI\x01] Disabling stats: less than \x034\x01 players.");
	} else {
		...
		PrintToChatAll("\x01[\x09KIWI\x01] Enabling stats: \x034\x01 or more players.");
	}
} 
...
public Action:ConCmd_Say(userid, args)
{
	...
	{
		if(strcmp(text[startidx], "!stats", false) == 0|| 
			strcmp(text[startidx], "!me", false) == 0)
		{
			g_player_count = GetPlayerCount();
			PrintStats(userid);
		}
...
public Action:AdminCmd_Purge(client, args)
{
	...
	if(argCount != 1)
	{
		PrintToConsole(client, "[KIWI Stats] Invalid number of arguments for command 'sm_stats_purge'");
		return Plugin_Handled;
	}
	...
	{
		PrintToConsole(client, "[KIWI Stats] Invalid arguments for sm_stats_purge.");
		return Plugin_Handled;
	}
	...
	if(days <= 0)
	{
		PrintToConsole(client, "[KIWI Stats] Invalid number of days.");
		return Plugin_Handled;
	}
...
public SQL_PurgeCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		PrintToConsole(data, "[KIWI Stats] Purge successful");
	}
}
...
GetPlayerCount()
{
	...
	{
		LogError("[KIWI Stats] Error getting player count.");
		return 0;
	}
...
bool:HookEvents(gameid)
{
	switch(gameid)
	{
		...
		default: 
		{
			LogError("[KIWI Stats] Invalid gameid (%i).", g_gameid);
			return false;
		}
	}
	return true;
...
{
	if(hndl == INVALID_HANDLE)
		LogError("[KIWI Stats] Error saving player (%s)", error);
}
...
public SQL_CreatePlayerCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(hndl != INVALID_HANDLE)
	{
		...
		LogError("[KIWI Stats] SQL_CreatePlayerCallback failure: %s", error);
}
