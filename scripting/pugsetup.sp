...
public void OnPluginStart() {
    ...
    g_DoVoteForKnifeRoundDecisionCvar = CreateConVar("sm_pugsetup_vote_for_knife_round_decision", "1", "If 0, the first player to type .stay/.swap/.t/.ct will decide the round round winner decision - otherwise a majority vote will be used");
    ...
    g_MessagePrefixCvar = CreateConVar("sm_pugsetup_message_prefix", "[{ORANGE}KIWI{NORMAL}]", "The tag applied before plugin messages. If you want no tag, you can set an empty string here.");
    ...
    g_SnakeCaptainsCvar = CreateConVar("sm_pugsetup_snake_captain_picks", "1", "Whether captains will pick players in a \"snaked\" fashion rather than alternating, e.g. ABBAABBA rather than ABABABAB.");
    ...
    AddPugSetupCommand("info", Command_QuickPug, "QuickPUG info", Permission_All);
    LoadExtraAliases();
...
static void QuickPugMessage() {
    ...
    if (dt >= READY_COMMAND_HINT_TIME) {
        ...
        for (int i = 1; i <= MaxClients; i++) {
            if (IsPlayer(i) && !PugSetup_IsReady(i) && OnActiveTeam(i)) {
                PugSetup_Message(i, "Welcome to TENMANS! Remember to {GREEN}!ready");
            }
        }
    }
}
...
public Action Command_QuickPug(int client, int args) {
    // PugSetup_Message(client, "A QuickPUG is a 12K start money, 10 round competitive PUG in which the first team to 6 rounds wins. Overtimes are best of 4 rounds with 8K starting money.");
    return Plugin_Handled;
}
