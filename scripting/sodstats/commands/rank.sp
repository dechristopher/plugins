...
public Rank_PlayerIdCallback(const String:name[], const String:steamid[], any:stats[], any:data, error)
{
	...
	{
		LogError("[KIWI Stats] RankCallback: Player not found");
		return;
	}
	...
	{
		Format(text, sizeof(text), "\x01[\x09KIWI\x01] Player \x03%s\x01's rank is \x04%i/%i\x01 with \x04%i\x01 points (%i to next rank), \x04%i\x01 kills and \x04%i\x01 deaths", 
								   name, 
								   rank, 
								   g_player_count, 
	...
	{
		new Float:KDRate = float(stats[STAT_KILLS])/float(stats[STAT_DEATHS]);
		// \x04[KIWI Stats] \x01Player \x03%s\x01's rank is \x04%i/%i\x01 with \x04%i\x01 points (%i to next rank), \x04%i\x01 kills and \x04%i\x01 deaths
		// [KIWI Stats] Player %s's rank is %i/%i with %i points (%i to next rank), %i kills and %i deaths
		Format(text, sizeof(text), "\x01[\x09KIWI\x01] \x03%s\x01 is #\x04%i\x01/\x04%i\x01 (\x04%i\x01 points, \x04%i\x01 to next) (\x04%i\x01k \x07%i\x01d \x04%.2f\x01KD)", 
								   name, 
								   rank, 
								   g_player_count, 
								   stats[STAT_SCORE] + g_start_points, 
