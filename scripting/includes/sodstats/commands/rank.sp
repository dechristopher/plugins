...
public Rank_PlayerIdCallback(const String:name[], const String:steamid[], any:stats[], any:data, error)
{
	...
	{
		Format(text, sizeof(text), "\x04KIWI-Stats: \x01Player \x03%s\x01's rank is \x04%i/%i\x01 with \x04%i\x01 points (%i to next rank), \x04%i\x01 kills and \x04%i\x01 deaths", 
								   name, 
								   rank, 
								   g_player_count, 
	...
	{
		Format(text, sizeof(text), "KIWI-Stats: Player %s's rank is %i/%i with %i points (%i to next rank), %i kills and %i deaths", 
								   name, 
								   rank, 
								   g_player_count, 
								   stats[STAT_SCORE] + g_start_points, 
