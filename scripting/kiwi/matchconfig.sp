...
#define CONFIG_MAPSTOWIN_DEFAULT 1
#define CONFIG_BO2_DEFAULT false
#define CONFIG_SKIPVETO_DEFAULT true
#define CONFIG_SIDETYPE_DEFAULT "always_knife"
...
public bool LoadMatchConfig(const char[] config) {
    ...
    if (g_CheckAuthsCvar.IntValue == 0 &&
        ...
        LogError("Setting player auths in the \"players\" section has no impact with kiwi_check_auths 0");
    }
...
public int System2_OnMatchConfigReceived(bool finished, const char[] error, float dltotal,
    ...
    if (!StrEqual(error, "")) {
        MatchConfigFail("Error receiving remote config vis system2: %s", error);
    }else if (finished) {
        LoadMatchConfig(REMOTE_CONFIG_FILENAME);
    }
}
...
public Action Command_LoadTeam(int client, int args) {
    ...
    if (args >= 2 && GetCmdArg(1, arg1, sizeof(arg1)) && GetCmdArg(2, arg2, sizeof(arg2))) {
        ...
        ReplyToCommand(client, "Usage: kiwi_loadteam <team1|team2|spec> <filename>");
    }
...
public Action Command_AddPlayer(int client, int args) {
    ...
    if (args >= 2 && GetCmdArg(1, auth, sizeof(auth)) && GetCmdArg(2, teamString, sizeof(teamString))) {
        ...
        ReplyToCommand(client, "Usage: kiwi_addplayer <auth> <team1|team2|spec>");
    }
    return Plugin_Handled;
}
...
public Action Command_RemovePlayer(int client, int args) {
    ...
    if (args >= 1 && GetCmdArg(1, auth, sizeof(auth))) {
        ...
        ReplyToCommand(client, "Usage: kiwi_removeplayer <auth>");
    }
    return Plugin_Handled;
}
...
public Action Command_CreateMatch(int client, int args) {
    ...
    Format(path, sizeof(path), "kiwi_%s.cfg", matchid);
