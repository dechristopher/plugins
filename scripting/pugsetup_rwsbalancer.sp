...
public Action Command_RWS(int client, int args) {
    ...
    if (args >= 1 && GetCmdArg(1, arg1, sizeof(arg1))) {
        ...
        if (target != -1) {
            if (HasStats(target))
                PugSetup_Message(client, "{PINK}%N{NORMAL} has an RWS of {GREEN}%.1f{NORMAL} with {GREEN}%d{NORMAL} rounds played",
                              target, g_PlayerRWS[target], g_PlayerRounds[target]);
            else
                PugSetup_Message(client, "{PINK}%N{NORMAL} does not currently have stats stored to calculate their RWS", target);
        }
    } else {
        PugSetup_Message(client, "Usage: {GREEN}.rws <{NORMAL}player{GREEN}>");
    }
