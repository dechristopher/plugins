...
public Action:Event_Say(client, const String:command[], arg)
{
	if (g_bEnabled)
	{
		static String:menuTriggers[][] = { "!knief", "!knifes", "!knfie", "!knifw", "!knifew", "!kinfe", "!kinfes", "knife", "/knif", "/knifes", "/knfie", "/knifw", "/knives", "/kinfe", "/kinfes", "!k", "/k" };
		...
		//Shadow Daggers
		decl String:Daggers[32];
		Format(Daggers, sizeof(Daggers), "%t", "Knife Trigger Daggers");
		if (StrEqual(text, Daggers, false))
		{
			if (IsValidClient(client))
			{
				if (HasPluginAccess(client)) {
...
{      
	...
	if (knife_choice[client] > 0) 
	{
		...
		if (!CheckCommandAccess(client, "sm_knifeupgrade_daggers", 0, true) && knife_choice[client] == 10) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}

		if (!CheckCommandAccess(client, "sm_knifeupgrade_golden", 0, true) && knife_choice[client] == 11) {
			knife_choice[client] = 0;
			CPrintToChat(client, "%t","No Longer Has Access to Current Knife");
		}
	}
	...
	switch(knife_choice[client]) {
		...
		case 11:iItem = GivePlayerItem(client, "weapon_knifegg");
		default: return;
	}
	if (iItem > 0) 
...
SetDaggers(client)
{
	...
	if (CheckCommandAccess(client, "sm_knifeupgrade_daggers", 0, true))
	{
		knife_choice[client] = 10;
		SetClientCookie(client, g_cookieKnife, "10");
		CreateTimer(g_fDelayedEquipTimer, CheckKnife, GetClientSerial(client), TIMER_FLAG_NO_MAPCHANGE);
		if (g_bKnifeChosenMessage)
			CPrintToChat(client, "%t","Daggers Given Message");
	} else {
		CPrintToChat(client, "%t", "Knife Access Denied Message");
	}
}
...
public Action:ShowKnifeMenu(client)
{
	...
	Format(Daggers, sizeof(Daggers), "%t", "Menu Knife Daggers");
	...
	//Daggers
	if (CheckCommandAccess(client, "sm_knifeupgrade_daggers", 0, true)) {
		AddMenuItem(menu, "option10", Daggers);
	} else {
		if(!g_bHideRestricted)
			AddMenuItem(menu, "option10", Daggers,ITEMDRAW_DISABLED);
	}
	//Golden
	if (CheckCommandAccess(client, "sm_knifeupgrade_golden", 0, true) && g_bEnableGoldenKnife) {
...
public ShowKnifeMenuHandler(Handle:menu, MenuAction:action, client, itemNum)
{
	switch(action){
		case MenuAction_Select: 
		{
			...
			//Daggers
			else if (strcmp(info,"option10") == 0) SetDaggers(client);
			//Golden
